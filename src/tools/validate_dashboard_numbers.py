...
def main() -> int:
    ...
    p = qm.PREFIX  # user-namespaced prefix
    pipe = r.pipeline(transaction=False)
    pipe.get(f"{p}counter:discovered")
    pipe.get(f"{p}counter:discovered_bytes")
    pipe.zcard(f"{p}queue:discovery")
    pipe.hlen(f"{p}failed")
    pipe.get(f"{p}counter:extraction_completed")
    for cat in ["tiny", "small", "medium", "large"]:
        pipe.zcard(f"{p}queue:extraction:{cat}")
    pipe.llen(f"{p}queue:indexing")
    pipe.zcard(f"{p}queue:ocr")
    pipe.llen(f"{p}queue:tagging")
    pipe.get(f"{p}counter:tagging_completed")
    pipe.get(f"{p}counter:root_completed")
    pipe.get(f"{p}counter:completed")
    pipe.get(f"{p}counter:completed_bytes")
    pipe.get(f"{p}counter:completed_extract_ms")
    pipe.get(f"{p}counter:completed_index_ms")
    pipe.get(f"{p}counter:duplicates")
    raw = pipe.execute()
    ...
    raw_ocr_completed = safe_int(r.get(f"{p}counter:ocr_completed"))
