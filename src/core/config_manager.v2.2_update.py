...
@dataclass
class VL16Config:
    """PaddleOCR-VL-1.6 configuration"""
...
class OCRConfig:
    ...
    engine: str = "tiered"
    vl16: VL16Config = field(default_factory=VL16Config)
...
class ConfigurationManager:
    ...
    def _load_config(self) -> None:
        ...
        # If app_root is "." or empty, derive it from the config file's parent directory
        # (config/config.yaml -> parent.parent = project root).
        app_root = self.raw_config.get('paths', {}).get('app_root', '')
        if not app_root or app_root == '.':
            app_root = str(self.config_path.resolve().parent.parent)
    ...
    def _create_config_objects(self) -> None:
        ...
        # Create OpenSearchConfig — append user namespace to index_name for isolation
        from core.constants import get_user_namespace
        os_raw = dict(self.raw_config['indexing']['opensearch'])
        os_raw['index_name'] = f"{os_raw['index_name']}_{get_user_namespace()}"
        opensearch = OpenSearchConfig(**os_raw)
        ...
        # Create VL16Config
        vl16 = VL16Config(**self.raw_config['ocr'].get('vl16', {}))
        ...
        ocr = OCRConfig(
            ...
            engine=self.raw_config['ocr'].get('engine', 'tiered'),
            tiered=self.raw_config['ocr'].get('tiered', None),
        )
