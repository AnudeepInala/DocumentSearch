...
from core.constants import (
    QueueStatus, SizeCategory, Priority, ErrorType, get_user_namespace
)
...
_USER_PREFIX = f"{get_user_namespace()}:docsearch:"
...
class RedisQueueManager:
    ...
    # Redis key prefixes — namespaced per OS user
    PREFIX = _USER_PREFIX
    ...
    # ------------------------------------------------------------------
    # OCR engine preference (set from dashboard, read by OCR workers)
    # ------------------------------------------------------------------
    _OCR_ENGINE_KEY = f"{_USER_PREFIX}config:ocr:engine"

    def set_ocr_engine(self, engine: str) -> None:
        """Persist the user-selected OCR engine ('vl16' or 'paddle') to Redis."""
        try:
            self.client.set(self._OCR_ENGINE_KEY, engine.strip().lower())
        except Exception as e:
            logger.warning(f"Could not save OCR engine preference to Redis: {e}")

    def get_ocr_engine(self) -> str:
        """Return the stored OCR engine preference, or '' if not set."""
        try:
            val = self.client.get(self._OCR_ENGINE_KEY)
            if val:
