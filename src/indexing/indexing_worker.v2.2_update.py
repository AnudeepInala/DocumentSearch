...
class IndexingWorker:
    ...
    def _process_batch(self, work_items: List[Dict[str, Any]]) -> None:
        ...
        for work_item in work_items:
            ...
            existing_ocr = self.os_client.get_document_ocr_fields(input_hash) if input_hash else None
            document = self.document_builder.build_document(work_item['document_json'], existing_ocr=existing_ocr)
            ...
            if document:
                ...
                metadata = {
                    ...
                    'needs_ocr': bool(document.get('needs_ocr', False)),
            ...
            try:
                ...
                for file_id in indexed_file_ids:
                    if file_id is not None:
                        meta = next(
                            (m for m in queue_metadata.values() if m.get('file_id') == file_id), {}
                        )
                        # Only mark completed here for files that don't need OCR.
                        # Files with needs_ocr=True are marked completed by the OCR worker
                        # after OCR finishes, so the "Fully Processed" count is accurate.
                        if not meta.get('needs_ocr', False):
