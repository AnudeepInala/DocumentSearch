...
class OpenSearchClient:
                    ...
                    body={
                        'script': {
                            'source': """
                                ...
                                if (ctx._source.containsKey('needs_ocr') && ctx._source.needs_ocr == true) {
                                    ctx._source.tika_content = ctx._source.main_content;
                                    ctx._source.main_content = params.ocr_content;
                                } else if (ctx._source.containsKey('main_content') == false || ctx._source.main_content == null || ctx._source.main_content == '') {
                                    ctx._source.main_content = params.ocr_content;
                                }
                            """,
                            'lang': 'painless',
    ...
    def get_document_ocr_fields(self, doc_id: str) -> Optional[Dict[str, Any]]:
        """Fetch only OCR fields for an existing document. Returns None if not found or no OCR."""
        try:
            response = self.client.get(
                index=self.index_name,
                id=doc_id,
                _source=['ocr_content', 'ocr_confidence', 'ocr_completed'],
            )
            src = response.get('_source', {})
            if src.get('ocr_content'):
                return src
            return None
        except NotFoundError:
            return None
